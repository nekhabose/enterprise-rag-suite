...
const signup = async (prefix) => {
  ...
  expect(response.data.user && response.data.user.id).toBeDefined();
  return {
...
const createTenant = async (namePrefix) => {
...
const assignUserTenantAndRole = async (userId, tenantId, role) => {
  await pool.query(
    `UPDATE users
...
const createDocumentForUser = async (userId, filename = 'seeded-doc.txt') => {
  const result = await pool.query(
...
const insertChunk = async (documentId, content, chunkIndex, seed) => {
...
const createSeededKnowledgeDoc = async (userId, namePrefix) => {
...
const expectStatus = (status, expected) => {
...
describe('RBAC - Role Permissions', () => {
  ...
  it('should deny student from deleting documents', async () => {
    ...
    expect(String(response.data && response.data.error || '')).toContain('Missing permission');
  });
...
describe('LLM Providers', () => {
  ...
  it('should list providers including Groq', async () => {
    ...
    expect(response.status).toBe(200);
    const providers = response.data && response.data.providers || {};
